...
func set_blood(value):
	...
	if blood == 0:
		...
		var effect_node = get_tree().get_root().get_node("GameScene").get_node("EffectNode")
		EffectMgr.create_virus_explode_effect(effect_node,global_position,cfg_data.scale)
