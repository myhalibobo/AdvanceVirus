...
func _on_speed_up(time):
	...
	if not timer.is_connected("timeout",self,"_on_speed_down"):
		timer.connect("timeout",self,"_on_speed_down",[randomf_from_to(1,1.5)])
...
func _on_speed_down(time):
	...
	if not timer.is_connected("timeout",self,"_on_speed_down"):
		timer.connect("timeout",self,"_on_speed_up",[randomf_from_to(1.5,2.5)])
	timer.wait_time = time
	timer.start()
